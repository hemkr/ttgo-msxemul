...
#define FAST_RDOP
extern byte *ROMPage[];
INLINE byte OpZ80(Z80_WORD A) { return(ROMPage[A>>13][A&0x1FFF]); }
#endif
...
INLINE byte RdZ80(Z80_WORD A) { return(Page[A>>13][A&0x1FFF]); }
//INLINE void WrZ80(Z80_WORD A,byte V) { if(Page[A>>13]<ROM) Page[A>>13][A&0x1FFF]=V; }
#endif
...
#define FAST_RDOP
extern byte *RAM[];
INLINE byte OpZ80(Z80_WORD A) { return(RAM[A>>13][A&0x1FFF]); }
#endif
...
#define WrZ80 WRZ80
extern byte *Page[],*ROM;
extern void (*DoMEM)(register Z80_WORD Addr,register byte V);
INLINE byte RdZ80(Z80_WORD A) { return(Page[A>>14][A&0x3FFF]); }
INLINE void WrZ80(Z80_WORD A,byte V) { if(Page[A>>14]<ROM) Page[A>>14][A&0x3FFF]=V; else DoMEM(A,V); }
#endif
...
/*************************************************************/
void IntZ80(Z80 *R,Z80_WORD Vector)
{
  ...
  if((R->IFF&IFF_1)||(Vector==INT_NMI))
  {
    ...
    if(R->IFF&IFF_IM2)
    {
      ...
      Vector=(Vector&0xFF)|((Z80_WORD)(R->I)<<8);
      /* Read the vector */
