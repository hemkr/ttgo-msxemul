...
// 1. 표준 라이브러리를 가장 먼저 포함
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// MSX.h에서 ARDUINO 섹션 수정
#ifdef ARDUINO
#include <stddef.h>
...
extern "C" {
  ...
  int   arduino_fgetc(void* stream);   // 이름 변경
  int   arduino_feof(void* stream);    // 이름 변경
#ifdef __cplusplus
}
#endif
...
// C 파일에서만 매크로 적용
#ifndef __cplusplus
#define fopen  arduino_fopen
#define fclose arduino_fclose
#define fread  arduino_fread
#define fwrite arduino_fwrite
#define fseek  arduino_fseek
#define ftell  arduino_ftell
#define rewind arduino_rewind
#define fgetc  arduino_fgetc  // 매크로로 이름 변경
#define feof   arduino_feof   // 매크로로 이름 변경
#endif
#endif
...
#ifdef ARDUINO
  // FabGL과의 충돌 방지를 위해 매크로 대신 인라인 함수나 다른 이름 사용 권장
  // 하지만 소스 수정 최소화를 위해 C++ 파일에서는 이 매크로를 숨김
  #ifndef __cplusplus
